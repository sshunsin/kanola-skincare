...
<html>
<head>
    <title>Edit Progress Product</title>
...
<div class="main-content">
    <h2 class="page-title">Edit Progress Product ✏️</h2>
    ...
    <div class="card">
        <form method="POST" action="{{ route('backend.v1.projects.progress.update',$projectProgress->id) }}">
            @csrf
            @method('PUT')

            <label>Kode</label>
            <input type="text" name="code" value="{{ $projectProgress->code }}" readonly style="background-color: #f3f4f6; cursor: not-allowed; border-color: #ffd1e8;">

            <label>Tahap</label>
            <input type="text" name="stage" value="{{ $projectProgress->stage }}" required>

            <label>Status</label>
            <select name="status" required>
                <option value="">-- Pilih Status --</option>
                <option value="planning" {{ strtolower($projectProgress->status) == 'planning' ? 'selected' : '' }}>Planning</option>
                <option value="not_started" {{ strtolower($projectProgress->status) == 'not_started' ? 'selected' : '' }}>Not Started</option>
                <option value="on_progress" {{ strtolower($projectProgress->status) == 'on_progress' ? 'selected' : '' }}>On Progress</option>
                <option value="completed" {{ strtolower($projectProgress->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                <option value="delayed" {{ strtolower($projectProgress->status) == 'delayed' ? 'selected' : '' }}>Delayed</option>
            </select>

            <label>Progress (%)</label>
            <input type="number" name="progress_percent" min="0" max="100"
                   value="{{ $projectProgress->progress_percent }}" required>

            <label>Tanggal Update</label>
            <input type="date" name="update_date" value="{{ $projectProgress->update_date }}" required>

            <label>Keterangan</label>
            <textarea name="description">{{ $projectProgress->description }}</textarea>

            <button class="btn btn-save" type="submit">
                <i class="fa-solid fa-floppy-disk"></i> Update Progress
            </button>
        </form>
    </div>
</div>
